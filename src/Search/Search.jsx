...
import healthCareCard from "../assets/health-care-card.png";
...
export default function Search() {
    ...
    useEffect(() => {
        const getHospitals = async () => {
            ...
            setIsLoading(true);
            try {
                const medicalCenter = await axios.get(`https://meddata-backend.onrender.com/data?state=${state}&city=${city}`);
                setHospitals(medicalCenter.data);
                setIsLoading(false);
            } catch (err) {
                console.log(err);
                setIsLoading(false);
        ...
        <>
            ...
            <Box sx={{ background: "linear-gradient(#EFF5FE, rgba(241,247,255,0.47))", width: "100%", pl: 0,}}>
                ...
                <Container maxWidth="xl" sx={{ pt: 8, pb: 10, px: { xs: 0, md: 4 } }}>
                    {hospitals.length > 0 && (
                        <Box sx={{ mb: 3 }}>
                            <Typography component="h1" fontSize={24} lineHeight={1.1} mb={2} fontWeight={500} ml={1}>
                    ...
                    <Stack alignItems="flex-start" direction={{ md: "row" }}>
                        <Stack mb={{ xs: 4, md: 0 }} spacing={3} width={{ xs: 1, md: "calc(100% - 384px)" }} mr="24px">
                            ...
                            {isLoading && (
                                <Typography variant="h3" bgcolor="#fff" p={3} borderRadius={2}>Loading...</Typography>
                            )}

                            {!state && (
                                <Typography variant="h3" bgcolor="#fff" p={3} borderRadius={2}>Please select a state and city</Typography>
                            )}
                        </Stack>

                        <img src={healthCareCard} width={360} height="auto" alt="banner" />
                    </Stack>
                </Container>
                <BookingModal
                    open={isModalOpen}
